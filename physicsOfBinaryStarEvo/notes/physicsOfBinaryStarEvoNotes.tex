...
\item Systems with \gls{ac:MSP}s generally have WD partners
...
\item \gls{ac:TII-SNe} 
\end{list}
\item due to the fact that \gls{ac:TIb-SNe} and \gls{ac:TIc-SNe} are stripped cores, it is incredibly likely that they result from binary systems
\item We may also observe both \gls{ac:TIb-SNe} and \gls{ac:TIc-SNe} in \gls{ac:WR-star}, however, we have \textbf{not} yet
\item WD SNe (section \ref{WD-SNe}) \textit{require} some sort of mass transfer process
\item The companion donor can be either a standard star (\gls{ac:SD}), or another WD (\gls{ac:DD})
\end{list}
...
\item Because of the high (\(\sim50000k\)) temps, they have strong stellar wind, and thus very similar spectra to \gls{ac:WR-star}, which they are distinguished from by the presence of hydrogen 
\item Denoted as \gls{ac:WNh}
\end{list}
...
\item Donors with radiative envelopes (\gls{ac:Case-A-RLO}) or slightly convective envelopes (early \gls{ac:Case-B-RLO}) will typically either shrink or stay the same radius
\item 
\end{list}
...
\item However, it is more likely for the star to eject its helium envelope due to helium-shell burning as well as the instability of the \gls{ac:RSG} stage, leaving behind a CO WD.
...
\item Generally, the carbon in the ore will ignite and leave a degenerate \gls{ac:ONeMg} core
...
\item Might also result in \gls{ac:TI-SNe}
\end{list}


\section{Effects of wind mass loss, metallicity, and rotation}
\begin{list}{-}{}
\item If a star has very fast spin, the helium in the core can get mixed into the whole star, preventing the star from becoming a giant, instead leading it towards becoming a \gls{ac:WR-star}\footnote{This is cool as shit. Blender star my beloved}. This can happen with stars with of low of mass as \(15M_\odot\), as compared to the typical progenitor mass of \(\sim25M_\odot\)
\item Non-rotating stars can become much more massive
% \item \gls{ac:RSG}s are much more common which stars of higher (sun-like) metallicities
\end{list}
